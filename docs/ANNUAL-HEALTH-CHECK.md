...
六姿態 A/B(配對 bootstrap,約 30 秒)。把結果表格完整記錄下來,
並與去年的 health 報告比對:
  - 有幾個姿態的「A−B 95% 區間」不跨 0?(去年是 0/6)
  - composite 的「A 贏機率」偏離 50% 多少?(去年 54%)
  - composite 的區間寬度有沒有縮小?(去年 [-0.35, 0.42])
  - 交易次數 A/B 有沒有改變?(去年 405/650)
明確指出變化,但不要動任何參數。
```

> ⚠️ **只看「95% 區間跨不跨 0」,不要比 Calmar 中位數的年度差異。**
> 中位數會因為抽樣而漂,2026 年就發生過同一份資料跑兩次結論翻盤。
> 唯一能說「A 真的比 B 好」的證據是**區間不跨 0**。
> 交易次數是確定性的,可以直接比。
